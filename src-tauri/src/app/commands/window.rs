...
#[command]
pub async fn operation_window(app: AppHandle, operation: WindowOperations, label: String) -> Result<(), String> {
    let window: Window = app.get_window(&label).ok_or_else(|| format!("窗口 '{}' 不存在", label))?;
